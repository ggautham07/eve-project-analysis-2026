...
try:
    ...
    print("Analysis file does not exist, creating...")

coordinates = []
for sim_dir in sim_dirs:
    coords = load_hdf5_file(path.join(sim_dir, "trajectories.h5"))["trajectory"]
    if flag_mult:
        coordinates.append(coords)
coordinates = np.concatenate(coordinates, axis=0) if flag_mult else coords[:]

R = np.linalg.norm(coordinates[..., 1, :] - coordinates[..., 0, :], axis=-1)

if flag_mult:
    makedirs(dir_results, exist_ok=True)
with File(path.join(dir_results, "analysis.h5"), mode="a") as handler:
    if "R(s)" in handler.keys():
        del handler["R(s)"]
    handler.create_dataset(name="R(s)", data=R[:])
...
plt.grid(lw=0.2)
plt.savefig(path.join(dir_results, "P(R).svg"), format="svg")
plt.close()
...
plt.savefig(path.join(dir_results, "R(s).svg"), format="svg")
plt.close()