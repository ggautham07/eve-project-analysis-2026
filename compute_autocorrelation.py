...
try:
    st = perf_counter()
    trajectory = load_hdf5_file(path.join(sim_dir, "E-P_trajectory.h5"))["trajectory"][start:]
...
autocorr_lim_all = []
tau = []
for m in range(len(linear_interlocus_dists)):
    # Get autocorrelation curve
    Rvecs = trajectory[:, m, 1, :] - trajectory[:, m, 0, :]
    ...
    print(f"Completed computation for loci pairs {linear_interlocus_dists[m]} kb")
...
handles = [patches.Patch(color=gregor_colours[i]+"98", label=f"{linear_interlocus_dists[i]} kb") for i in range(len(linear_interlocus_dists))]
...
tau_exp = [39.53656951044747103,
           ...
           742.8575441880935841]    # from David Brückner
slope1, intercept1 = np.polyfit(np.log10(linear_interlocus_dists), np.log10(tau_exp), deg=1)
slope2, intercept2 = np.polyfit(np.log10(linear_interlocus_dists), np.log10(tau), deg=1)
...
ax.scatter(linear_interlocus_dists, tau, s=400, marker="o", color=default_colours[0], label="Simulation")
...
# ax.plot(linear_interlocus_dists[:-1], (linear_interlocus_dists[:-1] ** 2) * 2,
