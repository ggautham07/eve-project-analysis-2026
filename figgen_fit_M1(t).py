...
#     plt.plot(dt_range_sim_log, sim_data_log[:, i], "o", markersize=8, label=f"Simulated data {i}")
#     plt.plot(dt_range_sim_log, sim_data_log_fit[:, i], "--", linewidth=2, label=f"Polynomial fit {i}")
...
for i in range(exp_data_log.shape[1]):
    ax.plot(10 ** (dt_range_exp_log + dTau_fit), 10 ** (exp_data_log[:, i] + dPhi_fit),
            "o-", markersize=15, linewidth=0.6, color=colours[i]+"99",
            markeredgecolor="#ff000080", markeredgewidth=0.85)
...
ax.plot(10 ** dt_range_sim_log[:20], (np.sqrt(10 ** (dt_range_sim_log[:20] + 0.5))), "--", linewidth=2.5, color="#000")
